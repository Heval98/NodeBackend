...
.put(authenticate.verifyOrdinaryUser, (req, res, next) => {
    Dishes.findById(req.params.dishId)
    .then((dish) => {
        var actualUserToken = authenticate.getToken({_id: req.user._id});
        var commentToken = authenticate.getToken({_id: dish.comments.id(req.params.commentId).author})
        if (dish != null && dish.comments.id(req.params.commentId) != null && commentToken === actualUserToken) {
            if (req.body.rating) {
        ...
        else if (commentToken !== actualUserToken) {
